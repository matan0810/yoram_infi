...
export default function Insights({ stats }) {
  ...
  const overdueTopics = useMemo(() => {
    ...
    return Object.entries(totalCount)
      .filter(([key, count]) => !EXCLUDED_TOPICS.has(key) && count >= 3 && maxYear - (lastSeen[key] || 0) >= 3)
  ...
  const recentTrend = useMemo(() => {
    ...
    return {
      entries: Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 6),
      total,
    };
  }, []);
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(340px,1fr))", gap: 20 }}>
          ...
          return (
            <InsightRow key={key}>
              <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                <Badge>{count}</Badge>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: "#c1440e" }}>
                    {TOPIC_HE[key]}
                  </div>
                  <div style={{ fontSize: 12, color: "#9b9890", marginTop: 1 }}>
                    {examCount}/{EXAMS.length} מבחנים · {Math.round((examCount / EXAMS.length) * 100)}%
                  </div>
                </div>
              </div>
            </InsightRow>
      ...
      <div style={card}>
        <CardTitle emoji="⚠️" title="מלכודות חוזרות" sub="שאלות כמעט זהות שחזרו מספר פעמים" />
        {TRAPS.map((trap, i) => (
          <InsightRow key={i}>
            <div style={{ fontWeight: 700, fontSize: 14, color: "#c1440e", marginBottom: 4 }}>
